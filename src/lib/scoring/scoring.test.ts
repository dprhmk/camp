...
import { distributeBalanced, distributeMembers, type DistributableMember } from "./distribute";
...
describe("distributeBalanced (randomised best-of-N)", () => {
  ...
  const make = (n: number): DistributableMember[] =>
    Array.from({ length: n }, (_, i) => ({
      ...
      physicalScore: (i % 5) + 1,
      mentalScore: ((i + 1) % 5) + 1,
      groups: [i % 2 === 0 ? "g:MALE" : "g:FEMALE"],
    }));

  it("assigns everyone and keeps sizes balanced with an rng", () => {
    const { squads, assignment } = distributeBalanced(make(40), 4, {
      rng: mulberry32(1),
      attempts: 10,
    });
    expect(Object.keys(assignment)).toHaveLength(40);
    const sizes = squads.map((s) => s.size);
    expect(Math.max(...sizes) - Math.min(...sizes)).toBeLessThanOrEqual(1);
  });

  it("can produce different splits for different random streams", () => {
    const a = distributeBalanced(make(40), 4, { rng: mulberry32(1), attempts: 8 }).assignment;
    const b = distributeBalanced(make(40), 4, { rng: mulberry32(999), attempts: 8 }).assignment;
